...
{
    public partial class ListaPrzedmiotow : Form
    {
        Element element;
        ...
        private void OdswierzListe()
        {
            ...
            string SQL = "SELECT id, nazwa, cena, poziom FROM przedmiot";
        ...
        {
            OknoPrzedmiot okno = new OknoPrzedmiot();
        ...
        private void btnUsun_Click(object sender, EventArgs e)
        {
            if (listView1.SelectedItems.Count > 0)
            {
                try
                {
                    ...
                    string sql = "DELETE FROM przedmiot WHERE id = " + item.Id;
                    MySqlCommand command = new MySqlCommand(sql, baza.Polaczenie);
        ...
        private void btnEdytuj_Click(object sender, EventArgs e)
        {
            ...
            {
                WierszViewItem item = (WierszViewItem)listView1.SelectedItems[0];
                OknoPrzedmiot okno = new OknoPrzedmiot(item.Id);
                okno.ShowDialog(this);
                if (okno.Sukces)
                {
                    OdswierzListe();
